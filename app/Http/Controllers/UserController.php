...
class UserController extends Controller
{
    ...
    {
        // Block district users from accessing user management
        $this->middleware(function ($request, $next) {
            if (auth()->check() && auth()->user()->district_id) {
                // Get the current action
                $action = $request->route()->getActionMethod();

                // Restricted actions for district users
                $restrictedActions = ['index', 'create', 'store', 'edit', 'update', 'destroy', 'toggleStatus'];
                ...
                if (in_array($action, $restrictedActions)) {
                    ...
                    if ($request->expectsJson() || $request->ajax()) {
                        return response()->json([
                            'success' => false,
                            'message' => 'Sizda foydalanuvchilarni boshqarish huquqi yo\'q. Faqat adminlar boshqarishi mumkin.'
                        ], 403);
                    }

                    // For web requests - redirect to dashboard instead of users.index
                    return redirect()->route('contracts.index')
                        ->with('error', 'Sizda foydalanuvchilarni ko\'rish huquqi yo\'q. Faqat adminlar ko\'rishi mumkin.');
                }
            }
    ...
    {
        $query = User::with('district'); // Add eager loading for district relationship
